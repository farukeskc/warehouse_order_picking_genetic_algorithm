...
namespace ComparisonOfOrderPickingAlgorithms
{
    public class Problem
    {
        ...
        public List<Item> getNonPickedAisleItems(int aPos, int bPos, bool isMidpoint, bool up)
        {
            List<Item> aisleItems = new List<Item>();

            if (isMidpoint)
            {
                double halfS = this.S / 2;

                int midpoint = Convert.ToInt32(Math.Ceiling(halfS));

                foreach (Item i in this.itemList)
                {
                    if (i.Picked == false && ((up && i.DInfo > midpoint) || (!up && i.DInfo <= midpoint)))
                    {
        ...
        {
            if (getNonPickedAisleItems(aPos, bPos, false, false) == null)
            {
                return 0;
            }
            return getNonPickedAisleItems(aPos, bPos, false, false).Count;
        }
