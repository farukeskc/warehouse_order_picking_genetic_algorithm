...
namespace ComparisonOfOrderPickingAlgorithms
{
    ...
    {
        public enum Algorithm { TabuSearch, SShape, LargestGap, GeneticAlgorithm, MidPoint };
        ...
        public dynamic solve(Algorithm method)
        {
            ...
            switch (method)
            {
                ...
                case Algorithm.GeneticAlgorithm:
                    return solveUsingGeneticAlgorithm(this.parameters.PickListGAParameters, new Item(0, this.problem.NumberOfCrossAisles - 1, 1, 0, this.problem.S));
                default:
        ...
        public void prepareDistanceMatrixForSpecificPickList(List<Item> items_input, ExcelWorksheet DMWorksheet)
        {
            List<Item> items = Utils.Clone<Item>(items_input);
            items.Add(new Item(0, this.Problem.NumberOfCrossAisles - 1, 1, 0, this.Problem.S));
            this.DistanceMatrix = new double[items.Count, items.Count];
            ...
            for (int i = 0; i < items.Count; i++)
            {
                for (int j = 0; j < items.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        Item targetItem = items[j];

                        int sourceItemMappingIndex = (40 - sourceItem.BInfo) + (sourceItem.CInfo == 0 ? 1 : 0) + (99 - sourceItem.DInfo) * (this.Problem.NumberOfAisles) - 1;
                        int targetItemMappingIndex = (40 - targetItem.BInfo) + (targetItem.CInfo == 0 ? 1 : 0) + (99 - targetItem.DInfo) * (this.Problem.NumberOfAisles) - 1;


                        if (!double.TryParse(DMWorksheet.Cells[sourceItemMappingIndex + 1, targetItemMappingIndex + 1].Value?.ToString(), out distance))
                        {
                            throw new System.Exception($"Invalid value at cell ({sourceItemMappingIndex},{targetItemMappingIndex}).");
                        }
                    }

                    this.DistanceMatrix[items[i].Index, items[j].Index] = distance;
                }
            }
        }
        ...
        public void importDistanceMatrix(String filePath, Item depot)
        {
            try
            {
                ...
                {
                    ExcelWorksheet worksheet = package.Workbook.Worksheets[0]; // Assuming the distance matrix is in the first worksheet
                    // int rowCount = worksheet.Dimension.Rows;
                    // int colCount = worksheet.Dimension.Columns;
                    
                    List<Item> allStops = Utils.Clone<Item>(this.Problem.ItemList);
                    allStops.Add(depot);

                    this.distances = new double[allStops.Count, allStops.Count];
                    ...
                    for (int i = 0; i < allStops.Count; i++)
                    {
                        for(int j = 0; j < allStops.Count; j++)
                        {
                            ...
                            {
                                Item sourceItem = allStops[i];
                                Item targetItem = allStops[j];

                                int sourceItemMappingIndex = sourceItem.BInfo + sourceItem.CInfo + (99 - sourceItem.DInfo) * (this.Problem.NumberOfAisles + 1);
                                int targetItemMappingIndex = targetItem.BInfo + sourceItem.CInfo + (99 - targetItem.DInfo) * (this.Problem.NumberOfAisles + 1);

                                
                                if (!double.TryParse(worksheet.Cells[sourceItemMappingIndex + 1, targetItemMappingIndex + 1].Value?.ToString(), out distance))
                                {
                                    throw new System.Exception($"Invalid value at cell ({sourceItemMappingIndex},{targetItemMappingIndex}).");
                                }
                            }

                            this.distances[allStops[i].Index, allStops[j].Index] = distance;
                        }
                    }
                }
                Console.WriteLine("Distance Matrix Imported");
            }
        ...
        private void printDepotFirstBestSolutionAndTotalDistance(int[] depotFirstBestSolution, bool log)
        {
            double totalDistance = 0;
            for (int i = 0; i < depotFirstBestSolution.Count() - 1; i++)
            {
                totalDistance += this.distances[depotFirstBestSolution[i], depotFirstBestSolution[i + 1]];
            }
            totalDistance += this.distances[depotFirstBestSolution[depotFirstBestSolution.Count() - 1], depotFirstBestSolution[0]];

            totalDistance += this.Problem.W + this.Problem.K;


            this.TravelledDistance = totalDistance;

            if (log)
            {
                for (int i = 0; i < depotFirstBestSolution.Count() - 1; i++)
                {
                    Console.Write("{0} -> ", depotFirstBestSolution[i]);
                }
                Console.Write("{0} -> ", depotFirstBestSolution[depotFirstBestSolution.Count() - 1]);
                Console.WriteLine("{0}", depotFirstBestSolution[0]);
                Console.WriteLine("DepotFirstBestSolutionTotalDistance:  {0}", totalDistance);
            }
        }
            ...
            for (int i = 0; i < depotFirstBestSolution.Length - 1; i++)
            {
                ...
                else
                {
                    ...
                    if (!currentItemToPick.Picked)
                    {
                        ...
                        {
                            picker.collectAisle(false, true, false, this.problem);
                        }
                        //picker will only pick item going up
                        else
                        {
                            picker.collectAisle(true, true, false, this.problem);
                        }
                    }
                }
            }
        ...
        public void solveUsingSShape()
        {
            ...
            if (isItOnlyOne)
            {
                picker.collectAisle(!picker.NextVerticalMove, true, false, this.problem);
        ...
        public void solveUsingLargestGap()
        {
            ...
            while (farMostBlock < this.problem.Depot.Y)
            {
                ...
                if (pickAisles.Count > 0)
                {
                    ...
                    for (int i = 0; i < pickAisles.Count-1; i++)
                    {
                        Aisle aisle = getLargestGapLimits(this.problem.getNonPickedAisleItems(picker.AInfo, pickAisles.ElementAt(i), false, false));
                    ...
                    {
                        picker.goToLocation(picker.AInfo, pickAisles.ElementAt(i), this.problem);
                        Aisle aisle = getLargestGapLimits(this.problem.getNonPickedAisleItems(picker.AInfo - 1, pickAisles.ElementAt(i), false, false));
                        picker.collectLGAisle(picker.NextVerticalMove, aisle, this.problem);
                    }
                }
                else
        ...
        public void solveUsingMidPoint()
        {
            picker.NextHorizontalMove = true;
            picker.NextVerticalMove = false;

            // Go to the leftmost pick aisle
            picker.goToLocation(picker.AInfo, this.problem.LeftPickAisle, this.problem);

            // Get the pick aisles of the farthest block
            List<int> pickAisles = this.problem.getPickAislesOfBlock(this.problem.FarthestBlock);
            ...
            if (isItOnlyOne)
            {
                // If there's only one aisle, collect it and move to the next block
                picker.collectAisle(!picker.NextVerticalMove, true, false, this.problem);
                farMostBlock = this.problem.FarthestBlock + 1;
            }
            else
            ...
            if (pickAisles.Count > 0)
            {
                ...
                {
                    picker.goToLocation(picker.AInfo, pickAisles.ElementAt(i), this.problem);
                    picker.collectAisle(picker.AInfo == 2, true, true, this.problem);
                }
            }
            ...
            picker.goToLocation(picker.AInfo, pickAisles.ElementAt(pickAisles.Count - 1), this.problem);
            // Go downward
            picker.goToLocation(2, pickAisles.ElementAt(pickAisles.Count - 1), this.problem);

            pickAisles.Reverse();

            if (pickAisles.Count > 0)
            {
                for (int i = 1; i < pickAisles.Count; i++)
                {
                    picker.goToLocation(picker.AInfo, pickAisles.ElementAt(i), this.problem);
                    picker.collectAisle(picker.AInfo == 2, true, true, this.problem);
                }
        ...
        public int[] solveUsingGeneticAlgorithm(PickListGAParameters parameters, Item depot)
        {
            stopWatch = Stopwatch.StartNew();
            //generating an initial solution list
            List<Item> initialSolutionList = generateInitialSolutionList(this.problem.ItemList, InitialSolutionType.Greedy, depot);
            ...
            //simulateDepotFirstBestSolution(depotFirstBestSolution);
            printDepotFirstBestSolutionAndTotalDistance(depotFirstBestSolution, false);
